...
const LandingPage = () => {
  ...
  const fetchFakeOrReal = async (text) => {
    try {
      ...
      const fallbackPrediction =
        fallbackData[0]?.fake_news_prediction === "True"
          ? "Real news"
          : "Fake news" || null;
  ...
  const fetchvalidate = async (text) => {
    try {
      ...
      let maxSimilarityValue = Math.max(
        ...validateResponse.map((article) => article.similarity)
      );
      let maxSimilarityValuePrecentage =
        Math.ceil(maxSimilarityValue * 10000) / 100 + " %";

      // Generate the HTML for the list of related news articles
      const relatedNewsHTML = validateResponse
        .map((article) => {
      ...
      document.getElementById(
        "accuracy"
      ).innerText = `Alignment Percentage: ${maxSimilarityValuePrecentage}`;
  ...
  const handleFakeNewsClickThrottled = throttle(handleFakeNewsClick, 5000);
    ...
    <div className="container">
      ...
      <div className="left-section">
        ...
        <div className="button-group">
          <Button type="primary" onClick={handleFakeNewsClickThrottled}>
            Check Fake News
          </Button>
      ...
      <div className="right-section">
        ...
        <p>
          <b>This News is:</b>{" "}
          <span className={isFake === "Real news" ? "positive" : "negative"}>
            {isFake}
          </span>
        </p>
        <p>
          <b>Sentiment:</b>{" "}
          <span className={isPositive === "Positive" ? "positive" : "negative"}>
            {isPositive}
          </span>
        </p>
